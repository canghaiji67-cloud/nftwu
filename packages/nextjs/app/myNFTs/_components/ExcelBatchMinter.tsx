...
export const ExcelBatchMinter = () => {
  ...
  const [uploadUnmatched, setUploadUnmatched] = useState<{ name: string; image_file: string }[]>([]);
  ...
  const handleBatchUpload = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setUploadUnmatched(result.unmatched || []);
        notification.remove(notificationId);
        const matched = (result.results || []).length;
        const unmatched = (result.unmatched || []).length;
        notification.success(`成功上传 ${matched} 个NFT元数据${unmatched > 0 ? `，未匹配图片 ${unmatched} 个` : ""}！`);
      } else {
  ...
  const handleBatchMint = async () => {
    ...
    const notificationId = notification.loading(`开始批量铸造 ${uploadResults.length} 个NFT...`);
      ...
      <div className="card-body">
        ...
        {uploadUnmatched.length > 0 && (
          <div className="alert alert-warning mt-2">
            <div className="font-bold">⚠️ 以下CSV中的图片未找到：</div>
            <div className="text-xs mt-1 space-y-1">
              {uploadUnmatched.slice(0, 10).map((u, idx) => (
                <div key={`${u.image_file}-${idx}`}>
                  {u.name || "(未命名)"} → {u.image_file}
                </div>
              ))}
              {uploadUnmatched.length > 10 && <div>... 其余 {uploadUnmatched.length - 10} 项</div>}
            </div>
            <div className="text-xs mt-2 opacity-70">
              提示：请确保CSV中的文件名与选择的图片文件完全一致（不含路径，大小写一致，无前后空格）。
            </div>
          </div>
        )}
